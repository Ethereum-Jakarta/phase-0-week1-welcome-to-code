...
function balikKata(kata) {
    ...
    let atak = kata.split('').reverse().join('');
    return atak;
}
...
console.log(konversiMenit(1200)); // 2:00
...
function xo(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (dictChar[currentChar]) {
            dictChar[currentChar] ++;
        } else {
            dictChar[currentChar] = 1;
        }
    }  
    const values = Object.values(dictChar);
